...
<script>
MathJax.typesetPromise(document.querySelectorAll('.mathjax-caption'));
</script>
...
| **Method**                                     | **Input** | **CT ($$\tau=0.1$$) ID ($$\uparrow$$)**      | **CT ($$\tau=0.1$$) OOD ($$\uparrow$$)**     | **$$\bar{Q}$$ ID ($$\downarrow$$)**    | **$$\bar{Q}$$ OOD ($$\downarrow$$)**   | 
| ---------------------------------------------- | --------- | ------------------ | ------------------ | ---------------- | ---------------- |
...
<figure style="text-align: center;">
    ...
    <figcaption style="color: black; font-size: 14px; margin-top: 8px;">
    Figure 6: <strong>Left:</strong> Turbulence-intensity spectrum averaged over time and <strong>OOD</strong> simulations for different 5D neural surrogates. Competitors tend to underestimate while GyroSwin matches the spectrum well with a slight discrepancy on higher frequencies. <strong>Right:</strong> Time-averaged zonal flow profile for a slice along $s$ across radial coordinates $x$ for a selected <strong>OOD</strong> simulation. GyroSwin captures the zonal flow profile.
    </figcaption>
</figure>
...
## Resources
 ...
 - Paper: https://arxiv.org/abs/2510.07314
